...
export async function setEntryEnabled(bookName: string, uid: number, enabled: boolean): Promise<void> {
  ...
  await ST.saveWorldInfo(bookName, data);
}
...
export async function addEntries(
  ...
  if (!ST.world_names.includes(bookName)) {
  ...
  await ST.saveWorldInfo(bookName, { entries: entryMap });
  return written;
}

/** 读取/设置世界书全局激活清单（供“把永久库加入全局启用”使用） */
export function setGlobalSelection(list: string[]): void {
  ST.selected_world_info.splice(0, ST.selected_world_info.length, ...list);
  Object.assign(ST.getWorldInfoSettings().world_info, { globalSelect: list });
